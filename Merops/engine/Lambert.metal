...
vertex
VertexOut lambertVertex(VertexInput in [[stage_in]], constant VertexUniforms & uniforms [[buffer(1)]])
{
    VertexOut out;
...
fragment
half4 lambertFragment(VertexOut in [[stage_in]], texture2d<float> texture [[texture(0)]])
{
    constexpr sampler defaultSampler;
    auto color = texture.sample(defaultSampler, in.texcoord) + 0.1;
    float diffuseFactor = saturate(dot(in.normal, -lightDirection));
    return half4(color * diffuseFactor) * (1 - in.wireColor.a) + half4(in.wireColor *in.wireColor.a);
}

fragment
half4 normalMapFragment(VertexOut in [[stage_in]],
...
vertex
BumpOut bumpVertex(VertexInput in [[stage_in]], constant VertexUniforms & uniforms [[buffer(1)]])
{
    BumpOut out;
    out.position = uniforms.projectionViewMatrix * float4(in.position, 1);
    out.texcoord = float2(in.texcoord.x, in.texcoord.y);
...
fragment
half4 bumpFragment(BumpOut in [[stage_in]], texture2d<float> texture [[texture(0)]], texture2d<float> normalmap [[texture(1)]])
{
    constexpr sampler defaultSampler;
